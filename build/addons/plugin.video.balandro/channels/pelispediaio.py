...
def list_all(item):
    ...
    if item.group == 'cupo':
        bloque = scrapertools.find_single_match(data, '</i> Películas online</h2>(.*?)>© PelisPedia')
    else:
        bloque = scrapertools.find_single_match(data, '<h1(.*?)>© PelisPedia')
        if not bloque: bloque = scrapertools.find_single_match(data, 'Series nuevas(.*?)>© PelisPedia')
    ...
    if item.page == 0:
        ...
        if sum_parts > 250:
            if platformtools.dialog_yesno(item.contentSerieName.replace('&#038;', '&').replace('&#8217;', "'"), '¿ Hay [COLOR yellow][B]' + str(sum_parts) + '[/B][/COLOR] elementos disponibles, desea cargarlos en bloques de [COLOR cyan][B]250[/B][/COLOR] elementos?'):
                platformtools.dialog_notification('PelispediaIo', '[COLOR cyan]Cargando elementos[/COLOR]')
                item.perpage = 250
...
def list_search(item):
    ...
    bloque = scrapertools.find_single_match(data, '<div class="close-results">(.*?)</div></div>')
