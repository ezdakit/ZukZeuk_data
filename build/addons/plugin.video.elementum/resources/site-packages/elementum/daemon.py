...
def start_elementumd(**kwargs):
    ...
    if hasSubprocess:
        ...
        repoAddonId = 'repository.elementum'
        serviceRepoAddonId = 'repository.service.elementum'
        repoAddonInfoFormat = "<addon name='{name}' id='{id}' provider-name='elgatito' version='0.1.860040'><extension name='{name}' point='xbmc.addon.repository'><checksum>{dataDir}plugin.video.elementum/addons.xml.md5</checksum><datadir zip='true'>{dataDir}</datadir><info compressed='false'>{dataDir}plugin.video.elementum/addons.xml</info></extension><extension point='xbmc.addon.metadata'><platform>all</platform><summary lang='en'>{summary}</summary></extension><requires></requires></addon>"
        repoAddonInfo = repoAddonInfoFormat.format(
            name = 'Elementum Repository',
            id = repoAddonId,
            dataDir = ELEMENTUMD_HOST + '/repository/elgatito/',
        ...
        serviceRepoAddonInfo = repoAddonInfoFormat.format(
            ...
            dataDir = 'https://StefanIlchev.github.io/ElementumService/',
        ...
        xbmc.executebuiltin('StartAndroidActivity(service.elementum.android, android.intent.action.MAIN, , version:0.1.860040%s)' % dataSuffix)
        for id in [serviceRepoAddonId]:
            try:
                from elementum.provider import parse_json
                detailsJson = '{"jsonrpc":"2.0","id":"1","method":"Addons.GetAddonDetails","params":{"addonid":"%s","properties":["installed","enabled"]}}' % id
                enableJson = '{"jsonrpc":"2.0","id":"1","method":"Addons.SetAddonEnabled","params":{"addonid":"%s","enabled":true}}' % id
